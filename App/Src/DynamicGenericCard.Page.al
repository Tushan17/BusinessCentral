page 50006 "Dynamic Generic Card"
{
    ...
    /// <summary>
    /// Called from Dynamic Generic List before RunModal. Stores the source table ID and row
    /// number, then reads field values from the source table via RecordRef and FieldRef.
    /// Also filters the Integer source table to a single row (Number=gRowNum) so the card
    /// displays exactly one record.
    /// </summary>
    ...
    begin
        ...
        // Restrict Integer source to the single row number that was selected
        Rec.SetRange(Number, gRowNum);
        UpdateColumnConfig();
        LoadFieldValues();
    end;
